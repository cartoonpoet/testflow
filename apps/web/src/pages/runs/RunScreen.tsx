 ...
 */
export function RunLiveProgress({ run }: { run: RunDetail }) {
  const { summary } = run;
  const isRunning = run.status === "running" || run.status === "queued";
  const current = currentStep(run);

  return (
    <div data-slot="live-progress" className="flex items-center gap-[12px] text-[11px]">
      ...
      */}
      <span className="min-w-0 flex-1 truncate text-run-summary-ink">
        {current === undefined ? "단계 정보 대기 중" : current}
      </span>
    </div>
  );
}
...
/** 지금 도는(또는 마지막으로 돈) 스텝 이름. 없으면 undefined. */
function currentStep(run: RunDetail): string | undefined {
  const running = [...run.steps].reverse().find((step) => step.status === "running");
  return (running ?? run.steps.at(-1))?.nameSnapshot;
}
